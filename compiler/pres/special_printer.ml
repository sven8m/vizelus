...
let xml_characters s = 
	String.fold_right (fun c new_s ->
		match c with
		| '<' -> "&lt;"^new_s
		| '>' -> "&gt;"^new_s
		| '&' -> "&amp;"^new_s
		| '\n' -> "&#xA;"^new_s
		| '"' -> "&quot;"^new_s
		| _ -> (String.make 1 c)^new_s
	) s ""

let endl_characters s = 
	String.fold_right (fun c new_s ->
		match c with
		| '\n' -> "&#xA;"^new_s
		| _ -> (String.make 1 c) ^ new_s
...
let longname ff ln =
  ...
  match ln with
    ...
    | Lident.Modname(qual) -> if qual.qual <> !filename then qualident ff qual else shortname ff qual.id

let name ff n = shortname ff (Zident.source n) (*do not show the number *)
...
let rec pattern ff ({ p_typ = ty; p_caus = caus_list } as pat) =
  let rec pattern ff pat =
    ...
    | Evarpat(n) -> fprintf ff "@[%a@]" name n (*not showing types *)
    ...
    | Etypeconstraintpat(p,_) ->
        fprintf ff "@[%a@]" pattern p (*not showing types *)
...
let basic_operations = ["mod";"||";"<>";"&&";"=";">";"<";"/";"and";"or";"+";"-";"/.";"+.";"-.";"*";"*.";">=";"<=";"~-.";"~-"]

let translate_basic_operation op = 
	match op with
	| "~-." -> "-"
	| "/." -> "/"
	| "+." -> "+"
...
let rec applications ff e = 
	match e.e_desc with
	| Eapp({ app_inline = i; app_statefull = r }, e, e_list) ->
		let change , name = match e.e_desc with
		| Eglobal { lname = ln} ->
			if (List.length e_list = 2) && List.mem (Lident.source ln) basic_operations then
				true, (xml_characters (translate_basic_operation (Lident.source ln)))
			else 
				false , ""
		| _ -> false , ""
		in

		let change1, name1 = match e.e_desc with
		| Eglobal {lname = ln} ->
			if (List.length e_list = 1) && List.mem (Lident.source ln) basic_operations then
				true, (xml_characters (translate_basic_operation (Lident.source ln)))
			else 
				false,""
		| _ -> false, ""
		...
		if change then begin
			let e1 = List.hd e_list in
			let e2 = List.hd (List.tl e_list) in
			fprintf ff "@[(%s%s%a %s %a)@]"
				(if i then "inline " else "") (if r then "run " else "")
				expression e1 name expression e2
		end else if change1 then begin
			let e = List.hd e_list in
			fprintf ff "@[(%s%s%s %a)@]"
				(if i then "inline " else "") (if r then "run " else "")
				name1 expression e
		end else
...
and expression ff e =
  ...
  match e.e_desc with
    ...
    | Etypeconstraint(e, typ) -> (* don't show type constraints *)
